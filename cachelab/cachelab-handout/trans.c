...
void transpose_submit(int M, int N, int A[N][M], int B[M][N])
{
	...
	int iblock = 8;
	int jblock = 4;
	for(i = 0; i < N; i += iblock){
		for(j = 0; j < M; j += jblock){
			for(i1 = i; i1 < i+iblock && i1 < N; i1++){
				for(j1 = j; j1 < j+jblock && j1 < N; j1++){
					temp = A[i1][j1];
					B[j1][i1] = temp;
				}
			}
 ...
 */ 
char transpose_completed_desc[] = "Best solution";
void transpose_completed(int M, int N, int A[N][M], int B[M][N])
{
...
void registerFunctions()
{
    ...
    registerTransFunction(transpose_completed, transpose_completed_desc);
